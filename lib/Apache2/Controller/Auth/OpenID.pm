...
of a simple cookie-based session mechanism that shows the browser
...
in the session, so C<< $r->notes->{a2c_session_force_save >>
will be set.
...
class name out of C<< $r->notes->{a2c_dispatch_class} >> and 
...
it sets C<< $r->notes->{a2c_openid_logged_in} >> for this request
...
If redirected, it also stashes the POST body into
the session as C<< {a2c}{openid}{redirect_post} >>
but if you use this, you probably want to put some kind of form id
and a unique value for every post so you're sure you're
doing the right thing.  I don't know if this will work,
...
sub process {
    ...
    DEBUG sub { "Trying to slice in opts: ".Dump($lwp_opts_directive) };
    if (scalar @lwp_opt_keys) {
        @{$conf{lwp_opts}}{@lwp_opt_keys} = @{$lwp_opts_directive}{@lwp_opt_keys};
    }

    $self->{a2c_openid_conf} = \%conf;

    DEBUG sub { "a2c_openid_conf:\n".Dump(\%conf) };
    DEBUG sub { "session:\n".Dump($self->pnotes->{session}) };
    ...
    if (!defined $openid_csr) {
        ...
        # we provide some junk if they didn't use the directive to specify
        # or generate some.
        $consumer_secret_string = $conf{consumer_secret} 
            || q|-qf_AD4#~a{~3)84cCvd+$6R89+,[l|;   
    ...
    my $dbh = $self->pnotes->{ $conf{dbi_name} }
        || a2cx "Database handle '$conf{dbi_name}' is not connected in pnotes.";

    my ($uname) = $dbh->selectrow_array(
        q{ SELECT uname FROM openid WHERE openid_url = ? },
        undef, $openid_url
    );
    ...
    if ($allow_login || ($vident = $openid_csr->verified_identity)) {
        ...
        # update the session
        $sess->{a2c}{openid}{logged_in} = 1;
        $sess->{a2c}{openid}{last_accessed_time} = time;
        $sess->{a2c}{openid}{remote_host} = $connection->get_remote_host();
        $sess->{a2c}{openid}{remote_ip}   = $connection->remote_ip();
        $sess->{a2c}{openid}{openid_url}  = $openid_url;

        # if everything works and we're returning the user okay,
        # make sure to delete the previous url from session hash
        delete $sess->{a2c}{openid}{previous_uri};

        # set a flag for this request so controller can print
        # a "login successful" message
        $self->notes->{a2c_openid_logged_in} = 1;
        ...
        my $return_uri = $sess->{a2c}{openid}{previous_uri};
        ...
        my $return_to = $self->construct_url($return_uri);
...
C<< session->{a2c}{openid}{previous_uri} >>.
...
sub redispatch {
    ...
    if  (   ($where_uri eq $register_uri || $where_uri eq $login_uri)
        ...
        $self->pnotes->{session}{a2c}{openid}{previous_uri} = $current_uri;
    }
  ...
  # DEBUG "loc is now '$loc'";
    ...
    my $dispatch_class = $self->notes->{a2c_dispatch_class}
        || a2cx 'No dispatch class saved in $r->notes->{a2c_dispatch_class}';
  ...
  # # clear the previously set response handler
    ...
    my $previously_set_controller = $self->notes->{controller}
        || a2cx "no controller previously dispatched in notes->{controller}";

    eval {
        my $redispatch_handler = $dispatch_class->new($self->{r});
        $redispatch_handler->find_controller;
        my $redispatch_controller = $self->notes->{controller}
            || a2cx "Redispatch set no new controller in notes->{controller}";
...
sub is_logged_in {
    ...
    my $openid_sess = $self->pnotes->{session}{a2c}{openid};
    ...
    return 1 if $timeout eq 'no timeout';
    ...
    if ($current_time - $timeout > $last_accessed) {
        ...
        $self->pnotes->{session}{a2c}{openid}{previous_uri} = $self->uri;
        return;
    }
...
sub logout {
    ...
    my $sess = $self->pnotes->{session};
    delete @{ $self->pnotes->{session}{a2c}{openid} }{qw(
        logged_in
        last_accessed_time
        openid_url
    )};
